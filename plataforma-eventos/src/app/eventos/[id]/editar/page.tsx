...
export default function EventoEditPage({ params }: EventoEditPageProps) {
    ...
    const fetchEvent = async () => {
      try {
        ...
        if (data.success) {
          setEvent(data.event)
          setFormData({
            sector: data.event.sector,
            categoria: data.event.categoria,
            titulo: data.event.titulo,
            descripcion: data.event.descripcion,
            fecha: data.event.fecha,
            horaInicio: data.event.horaInicio,
            horaFin: data.event.horaFin,
            ubicacion: data.event.ubicacion,
            numeroConvocados: data.event.numeroConvocados.toString(),
            contactos: data.event.contactos.length > 0 ? data.event.contactos : [{ id: '1', nombre: '', telefono: '', email: '' }],
            autoridades: data.event.autoridades.length > 0 ? data.event.autoridades : [{ id: '1', nombre: '', cargo: '' }]
          })
        } else {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="mb-8">
        <Link href={`/eventos/${params.id}`} className="inline-flex items-center text-primary hover:underline mb-4">
